...
# fmethod: univariate time-series forecasting method, ets or arima

clr_region_gap_fore <- function(state_index, year_index, horizon, fmethod)
{
	  ...
	  subnational_region_gap = CLR(female_val)$LR - CLR(Japan_female_dum[1:year_index,])$LR
    subnational_region_gap_fore = forecast(ftsm(fts(ages, t(subnational_region_gap))), h = horizon, method = fmethod)$mean$y[,horizon]
  	...
  	rm(subnational_region_gap); rm(subnational_region_gap_fore); rm(female_national_clr_fore); rm(female_subnational_clr_fore)
	
  	# male data
  	
  	subnational_region_gap = CLR(male_val)$LR - CLR(Japan_male_dum[1:year_index,])$LR
  	subnational_region_gap_fore = forecast(ftsm(fts(ages, t(subnational_region_gap))),h = horizon, method = fmethod)$mean$y[,horizon]
  	
  	male_national_clr_fore = forecast(ftsm(fts(ages, t(CLR(Japan_male_dum[1:year_index,])$LR))), h = horizon, method = fmethod)$mean$y[,horizon]
    ...
    rm(female_val); rm(male_val); rm(subnational_region_gap); rm(subnational_region_gap_fore); rm(male_national_clr_fore); rm(male_subnational_clr_fore)    
    return(list(male_fore = male_den_fore, female_fore = female_den_fore))
}

# region_index: prefecture index
...
clr_region_gap_fore_fun_int <- function(region_index, PDF_M_holdout_data, PDF_F_holdout_data, horizon, uni_fore_method, level_sig)
{
    ...
    {
        dum <- clr_region_gap_fore(state_index = region_index,
...
clr_region_gap_ets_int_F = clr_region_gap_ets_int_M = array(NA, dim = c(n_state, 20, 3), dimnames = list(state, 1:20, c("ECP", "CPD", "MIS")))
for(iwj in 1:n_state)
{
    for(iwk in 1:20)
    {
        dum = clr_region_gap_fore_fun_int(region_index = iwj, PDF_M_holdout_data = male_prefecture_dx[[iwj]], 
      	                                  PDF_F_holdout_data = female_prefecture_dx[[iwj]],
          	                              horizon = iwk, uni_fore_method = "ets", level_sig = 0.8)
        clr_region_gap_ets_int_F[iwj,iwk,] = dum$int_F_err
        clr_region_gap_ets_int_M[iwj,iwk,] = dum$int_M_err
        rm(iwk)
    }
    print(iwj); rm(iwj)
}

horizon_clr_region_gap_ets_int_F_err_mean = apply(clr_region_gap_ets_int_F, c(2, 3), mean)
horizon_clr_region_gap_ets_int_M_err_mean = apply(clr_region_gap_ets_int_M, c(2, 3), mean)

clr_region_gap_ets_int_F_err_mean = apply(clr_region_gap_ets_int_F, c(1, 3), mean)
clr_region_gap_ets_int_M_err_mean = apply(clr_region_gap_ets_int_M, c(1, 3), mean)

# fmethod = "arima"

clr_region_gap_arima_int_F = clr_region_gap_arima_int_M = array(NA, dim = c(n_state, 20, 3), dimnames = list(state, 1:20, c("ECP", "CPD", "MIS")))
for(iwj in 1:n_state)
{
    for(iwk in 1:20)
    {
        ...
        clr_region_gap_arima_int_F[iwj,iwk,] = dum$int_F_err
        clr_region_gap_arima_int_M[iwj,iwk,] = dum$int_M_err
        rm(iwk)
    }
    print(iwj); rm(iwj)
}

horizon_clr_region_gap_arima_int_F_err_mean = apply(clr_region_gap_arima_int_F, c(2, 3), mean)
horizon_clr_region_gap_arima_int_M_err_mean = apply(clr_region_gap_arima_int_M, c(2, 3), mean)

clr_region_gap_arima_int_F_err_mean = apply(clr_region_gap_arima_int_F, c(1, 3), mean)
clr_region_gap_arima_int_M_err_mean = apply(clr_region_gap_arima_int_M, c(1, 3), mean)
...
clr_region_gap_ets_int_F_95 = clr_region_gap_ets_int_M_95 = array(NA, dim = c(n_state, 20, 3), dimnames = list(state, 1:20, c("ECP", "CPD", "MIS")))
for(iwj in 1:n_state)
{
    for(iwk in 1:20)
    {
        ...
        clr_region_gap_ets_int_F_95[iwj,iwk,] = dum$int_F_err
        clr_region_gap_ets_int_M_95[iwj,iwk,] = dum$int_M_err
        rm(iwk)
    }
    print(iwj); rm(iwj)
}

horizon_clr_region_gap_ets_int_F_95_err_mean = apply(clr_region_gap_ets_int_F_95, c(2, 3), mean)
horizon_clr_region_gap_ets_int_M_95_err_mean = apply(clr_region_gap_ets_int_M_95, c(2, 3), mean)

clr_region_gap_ets_int_F_95_err_mean = apply(clr_region_gap_ets_int_F_95, c(1, 3), mean)
clr_region_gap_ets_int_M_95_err_mean = apply(clr_region_gap_ets_int_M_95, c(1, 3), mean)


# fmethod = "arima"

clr_region_gap_arima_int_F_95 = clr_region_gap_arima_int_M_95 = array(NA, dim = c(n_state, 20, 3), dimnames = list(state, 1:20, c("ECP", "CPD", "MIS")))
for(iwj in 1:n_state)
{
    for(iwk in 1:20)
    {
        ...
        clr_region_gap_arima_int_F_95[iwj,iwk,] = dum$int_F_err
        clr_region_gap_arima_int_M_95[iwj,iwk,] = dum$int_M_err
        rm(iwk)
    }
    print(iwj); rm(iwj)
}

horizon_clr_region_gap_arima_int_F_95_err_mean = apply(clr_region_gap_arima_int_F_95, c(2, 3), mean)
horizon_clr_region_gap_arima_int_M_95_err_mean = apply(clr_region_gap_arima_int_M_95, c(2, 3), mean)

clr_region_gap_arima_int_F_95_err_mean = apply(clr_region_gap_arima_int_F_95, c(1, 3), mean)
clr_region_gap_arima_int_M_95_err_mean = apply(clr_region_gap_arima_int_M_95, c(1, 3), mean)
